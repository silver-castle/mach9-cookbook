...
Mach9 handler functions must be defined using the `async def` syntax, as they
are asynchronous functions.

## Request parameters

Mach9 comes with a basic router that supports request parameters.
...
inside the quotes. If the parameter does not match the specified type, Mach9
...
Mach9 provides a `url_for` method, to generate URLs based on the handler method name. This is useful if you want to avoid hardcoding url paths into your app; instead, you can just reference the handler name. For example:
